...
import static me.weishu.kernelsu.KernelSUApplicationKt.ksuApp;
...
public class NetworkUtils {
    private static Context mContext = ksuApp;
     ...
     * @return {@code true}: 是<br>{@code false}: 否
     */
    public static boolean isConnectedReal() {
        if (getDataEnabled()) {
            return true;
        } else if (isWifiConnected()) {
            return (isAvailableByPing()) ? true : false;
        }
        return isConnected() && isWifiAvailable();
    }
    ...
    public static String getIPAddress(boolean useIPv4) {
        try {
            for (Enumeration<NetworkInterface> nis = NetworkInterface.getNetworkInterfaces(); nis.hasMoreElements(); ) {
                NetworkInterface ni = nis.nextElement();                // 防止小米手机返回10.0.2.15
                if (!ni.isUp()) continue;
